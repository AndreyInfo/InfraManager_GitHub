define(['knockout', 'jquery', 'ajax', 'ttControl', 'urlManager'], function (ko, $, ajaxLib, tclib, urlManager) {
    var module = {
        ...
        param_mode: 'mode',
        //
        ViewModel: function (availability) {
            var self = this;
            //
            self.modes = module.modes;
            self.Dictionaries = ko.observable(null); //модель справочников
            //
            ...
            self.CheckData = function () {//загрузка / перезагрузка вкладки
                ...
                if (activeMode == module.modes.dictionaries) {
                    if (self.Dictionaries() == null) {
                        ...
                        require(['ui_lists/Settings/Dictionaries'], function (vm) {
                            var mod = new vm.ViewModel(availability);
                            self.Dictionaries(mod);
                            mod.CheckData();
                            //
                            self.ViewTemplateName('../UI/Lists/Settings/Dictionaries');
                            hs();
                        });
                    }
                    else {
