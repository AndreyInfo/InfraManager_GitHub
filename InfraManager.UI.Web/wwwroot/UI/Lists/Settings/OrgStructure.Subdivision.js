define(['knockout', 'jquery', 'ajax', './OrgStructure.SubdivisionList', './OrgStructure.Attachments', './OrgStructure.Location', './OrgStructure.CalendarWorkSchedule', 'ui_controls/ContextMenu/ko.ContextMenu'],
    function (ko, $, ajaxLib, subdivisionList, attachments, location, schedule) {
        var module = {
            ...
            ViewModel: function (node, parentNode, selectDivision, mode) {//общая модель представления
                ...
                self.ClassID = !!getObjectClassID(node) ? getObjectClassID(node) : 102;
                ...
                self.ParentSubdivision = ko.observable(null);
                self.HasITService = ko.observable(false);
                ...
                //
                self.Location = ko.observable(mode != module.modes.create ? new location.ViewModel(self.Organization, self.ParentSubdivision, self.IsEditable) : null);
                //
                ...
                self.Organization.subscribe(function () {
                    self.MarkAsModified();
                });
                self.ParentSubdivision.subscribe(function () {
                    self.MarkAsModified();
                });
                self.HasITService.subscribe(function () {
                    self.MarkAsModified();
                });
                ...
                self.CheckData = function () {//вызывается извне, сообщает, что пора обновлять/загружать данные
                    if (self.ActiveMode() != module.modes.create) {
                        return self.InitNavigationBar().then(function () { return self.LoadSubdivision().then(self.ResetModified); });
                    } else if (!self.IconClass()) {
                        return self.InitIconClass().then(function () { return self.InitParents().then(self.ResetModified); });
                    }
                };
                ...
                self.CanSwitchToEditMode = ko.computed(function () {
                    return self.operationIsGranted(228, self.grantedOperations());
                });
                        ...
                        function (newVal) {
                            if (newVal && newVal.Result === 0) {
                                ...
                                require(['sweetAlert'], function () {
                                    swal(getTextResource('ErrorCaption'), getTextResource('NullParamsError') + '\n[OrgStructure.Subdivision.js, InitNavigationBar]', 'error');
                                    retD.resolve();
                                });
                    ...
                    self.ajaxControl.Ajax(self.$region,
                        ...
                        function (newVal) {
                            if (newVal && newVal.Result === 0) {
                                ...
                                self.ParentSubdivision(data.ParentSubdivision);
                                self.HasITService(data.HasITService);
                                ...
                                require(['sweetAlert'], function () {
                                    swal(getTextResource('ErrorCaption'), getTextResource('NullParamsError') + '\n[OrgStructure.Subdivision.js, LoadSubdivision]', 'error');
                                    retD.resolve();
                                });
                            else if (newVal && newVal.Result === 2)
                                require(['sweetAlert'], function () {
                ...
                self.InitParents = function () {
                    ...
                    if (self.ParentNode.ClassID == 101) {
                        self.Organization(self.ParentNode);
                        self.ParentSubdivision(null);
                        retD.resolve();
                        return retD.promise();
                    }
                    //
                    var data = {
                        ...
                        classID: self.ParentNode.ClassID,
                        type: 0 // OrgStructure
                    };
                    ...
                    self.ajaxControl.Ajax(self.$region,
                        ...
                        function (newVal) {
                            if (newVal && newVal.Result === 0) {
                                newVal.List.forEach(function (node) {
                                    if (node.ClassID == 101 && self.ParentNode.ClassID == 102) {
                                        self.Organization(node);
                                        self.ParentSubdivision(self.ParentNode);
                                        retD.resolve();
                                    }
                                });
                                retD.resolve();
                ...
                self.SaveSubdivision = function () {
                    ...
                    var data = {
                        ...
                        'Name': self.Name(),
                        'Note': self.Note(),
                        'HasITService': self.HasITService(),
                        'IsLockedForOSI': self.IsLockedForOSI(),
                        'OrganizationID': self.Organization().ID,
                        'ParentSubdivisionID': self.ParentSubdivision() && self.ParentSubdivision().ID,
                        'CalendarWorkScheduleID': self.CalendarWorkSchedule() && self.CalendarWorkSchedule().ID
                    };
                    var command = self.ActiveMode() == 'create' ? 'AddSubdivision' : 'EditSubdivision';
                    self.ajaxControl.Ajax(self.$region,
                        {
                            dataType: "json",
                            method: 'POST',
                        ...
                        function (newVal) {
                            if (newVal && newVal.Result === 0) {
                                ...
                                require(['sweetAlert'], function () {
                                    swal(getTextResource('ErrorCaption'), getTextResource('NullParamsError') + '\n[OrgStructure.Subdivision.js, ' + command + ']', 'error');
                                    retD.resolve();
                                });
                            else if (newVal && newVal.Result === 2)
                                require(['sweetAlert'], function () {
                ...
                self.SaveEditClick = function () {
                    return self.SaveSubdivision().then(function (savedSubdivision) {
                        if (savedSubdivision) {
                            self.ResetModified();
                            selectDivision(savedSubdivision, !!self.ParentNode ? self.ParentNode : self.Node);
                        }
                    });
                };
