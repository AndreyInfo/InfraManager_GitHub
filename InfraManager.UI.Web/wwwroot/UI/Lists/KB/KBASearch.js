    ...
    function (ko, $, ajaxLib, kbsLib, fc) {
        var module = {
            ...
            ViewModel: function () {
                ...
                //
                self.minWidth = 250;
                self.maxWidth = (navigator.userAgent.indexOf("Edg") != -1) ? 575 : 900;

                self.leftPanelWidth = ko.observable(500);
                ...
                {//ko.contextMenu
                    ...
                    self.contextMenuInit = function (contextMenu) {
                        ...
                        // Редактировать папку
                        self.editMenuItem(contextMenu, "ActionEdit");
                        // Добавить папку
                        self.addChildMenuItem(contextMenu, "Add_child_section");
                        // Добавить статью
                        self.addKbArticleMenuItem(contextMenu, "Add_article");
                        // Удалить папку
                        self.deleteMenuItem(contextMenu, "Delete_section");
                    };
                    //
                    ...
                    self.editMenuItem = function (contextMenu, restext) {
                        var isEnable = function () {
                            return self.currentFolder() && self.operationIsGranted(496) ? true : false;
                        };
                        var isVisible = function () {
                            return self.operationIsGranted(496) && self.currentFolder().ID != '00000000-0000-0000-0000-000000000000';//OPERATION_KBArticleFolder_Update
                        };
                        var action = function () {
                            if (!self.currentFolder())
                                return;
                            //
                            self.currentFolder().ShowFolderEditOption();
                        };
                        //
                    ...
                    //
                    self.addChildMenuItem = function (contextMenu, restext) {
                        var isEnable = function () {
                            return self.currentFolder() && self.operationIsGranted(494) ? true : false;
                        };
                        var isVisible = function () {
                            return self.operationIsGranted(494) && self.currentFolder().ID != '00000000-0000-0000-0000-000000000000';//OPERATION_KBArticleFolder_Add
                        };
                        var action = function () {
                            if (!self.currentFolder())
                                return;
                            //
                            self.currentFolder().ShowFolderAddChildOption();
                        };
                        //
                        var cmd = contextMenu.addContextMenuItem();
                        cmd.restext(restext);
                    ...
                    //
                    self.addKbArticleMenuItem = function (contextMenu, restext) {
                        var isEnable = function () {
                            return self.currentFolder() && self.operationIsGranted(490) ? true : false;
                        };
                        var isVisible = function () {
                            return self.operationIsGranted(490);//OPERATION_KBArticle_Add
                        };
                        var action = function () {
                            if (!self.currentFolder())
                                return;
                            //
                            self.CreateKbArticle();
                        };
                        //
                        var cmd = contextMenu.addContextMenuItem();
                        cmd.restext(restext);
                    ...
                    self.deleteMenuItem = function (contextMenu, restext) {
                        var isEnable = function () {
                            return self.currentFolder() && self.operationIsGranted(495) && self.kbaList().length === 0 ? true : false;
                        };
                        var isVisible = function () {
                            return self.operationIsGranted(495) && self.currentFolder().ID != '00000000-0000-0000-0000-000000000000';//OPERATION_KBArticleFolder_Delete
                        };
                        var action = function () {
                            if (!self.currentFolder())
                                return;
                            //
                            self.currentFolder().DeleteFolder();
                        };
                        //
                        var cmd = contextMenu.addContextMenuItem();
                        cmd.restext(restext);
                ...
                //
                self.ShowKBArticle = function (kbArticle, event) {
                    if (self.showContenHint() != "searchResult")
                        self.showContenHint(self.showContenHints.breadcrumbs);
                    self.showContent(self.showContentParam.KBArticle);
                    //
                    $.when(operationIsGrantedD(492)).done(function (can_update) {//OPERATION_KBArticle_Update
                        require(['ui_lists/KB/KBArticle'], function (lib) {
                            var mod = new lib.ViewModel(can_update, $('#knowlegdeBase'));
                            //
                            $.when(mod.Load(kbArticle.ID)).done(function () {
                                self.kbaView(mod);
                            });
                        });
                    });
            ...
            KBFolder: function (id, parentId, name, hasChilds, note, fullName, hasNumber, parentObject, self) {
                ...
                kself.isShowMenu = ko.observable(true);
                                        ...
                                        kself.ajaxControl.Ajax(null,
                                            {
                                                dataType: "json",
                                                method: 'DELETE',
                                                url: '/api/kb/DeleteKBFolder?' + $.param(param)
                                            },
                                            function (response) {
