define(['knockout', 'jquery', 'ajax'], function (ko, $, ajax) {
    var module = {
        Tab: function (vm) {
            ...
            //
            self.Name = getTextResource('LicenceScheme_HistoryTab');
            self.Template = '../UI/Forms/Settings/LicenceSchemes/frmLicenceScheme_historyTab';
                    ...
                    self.ajaxControl.Ajax(element,
                        {
                            dataType: "json",
                            method: 'GET',
                            url: '/api/' + vm.objectClassID + '/' + vm.object().ID() + '/events'
                        },
                        function (list) {
                            if (list) {
