define(['knockout',
    ...
    './templateParamsUpdate/UpdateCharacteristics(UpdateModel)'
],
    function (ko,
        $,
        ajaxLib,
        ...
        var module = {
            ViewModel: function ($region, selectedObjects) {
                ...
                self.Load = function () {
                    ...
                    self.SoftwareLicenceTypeName = getTextResource('SoftwareLicenceUpdateType_VersionUpdate');
                    self.GetContract(selectedObjects[0].ID);
                ...
                self.AfterRender = function () {
                    //self.frm.AfterRender();
                }

                self.GetContract = function (ID) {
                    var retval = $.Deferred();
                ...
                self.Upgrade = function () {
                    ...
                    let data = {
                        ...
                        'SoftwareModelID': self.UpdateCharacteristics().SoftwareModelObj().ID,
                        'Version': self.UpdateCharacteristics().SoftwareModelObj().Version
                    };
                    ...
                    self.ajaxControl.Ajax(self.$region,
                        {
                            ...
                            method: 'POST',
                            data: data,
                            url: '/assetApi/AssetLicenceUpgradeByContract'
                        },
                        function (newVal) {
                            if (newVal) {
                                if (newVal.Result === 0) {
            ...
            ShowDialog: function (selectedObjects, operationName, lifeCycleStateOperationID, isSpinnerActive) {
                ...
                $.when(userD).done(function (user) {
                    ...
                    frm = new fc.control(
                        'frmLicenceContractUpdate',//form region prefix
                        'frmLicenceContractUpdate_setting',//location and size setting
                    ...
                    vm.template().periodEndDateTime.subscribe(function (newValue) {
                        var newButtons = {}
                        if (newValue != null && vm.UpdateCharacteristics().SoftwareModelObj() != null) {
                            forceClose = false;
                    ...
                    vm.UpdateCharacteristics().SoftwareModelObj.subscribe(function (newValue) {
                        var newButtons = {}

                        if (newValue != null && vm.template().periodEndDateTime() != null) {
                            forceClose = false;
                            newButtons[operationName] = function () {
                                var d = vm.Upgrade();
                                $.when(d).done(function (result) {
