define(['knockout',
    ...
    './templateParamsUpdate/UpdateCharacteristics(Apply)'
],
    function (ko,
        $,
        ajaxLib,
        ...
        var module = {
            ViewModel: function ($region, selectedObjects, mode) {
                ...
                self.Load = function () {
                    ...
                    self.FilterProductCatalogTypeID = (self.selectedObjects[0].ProductCatalogTemplate === self.ProductCatalogTypeIDApply)
                        ? self.ProductCatalogTypeIDSubscription
                        : self.ProductCatalogTypeIDApply
                    //Активно поле выбора "Обновляемая лицензия" 
                    self.IsParentSelectable = ko.computed(function () {
                        return (self.IsEditMode()) ?
                            self.selectedObjects[0].ProductCatalogTemplate === self.ProductCatalogTypeIDApply
                            : false;
                    });
                    ...
                    self.IsChildSelectable = ko.computed(function () {
                        return (self.IsEditMode()) ?
                            self.selectedObjects[0].ProductCatalogTemplate === self.ProductCatalogTypeIDSubscription
                            : false;
                    });
                    ...
                    self.SoftwareLicenceTypeName = getTextResource('SoftwareLicenceUpdateType_SubscriptionRenewal');
                    ...
                    self.ajaxControl.Ajax(self.$region,
                        {
                            ...
                            method: 'POST',
                            data: data,
                            url: '/assetApi/AssetLicenceApply'
                        },
                        function (newVal) {
                            if (newVal) {
                                if (newVal.Result === 0) {
                                    ...
                                    //                                    
                                    var message = getTextResource('AssetLicenceApply_Sucsess');
                                    //
                                    var succsess = true;
                                    if (self.printReport()) {
            ...
            ShowDialog: function (selectedObjects, operationName, lifeCycleStateOperationID, isSpinnerActive, mode) {
                ...
                $.when(userD).done(function (user) {
                    ...
                    frm = new fc.control(
                        'frmLicenceApplying',//form region prefix
                        'frmLicenceApplying_setting',//location and size setting
                        ...
                        'data-bind="template: {name: \'../UI/Forms/Asset/AssetOperations/frmLicenceApplying\', afterRender: AfterRender}"'//attributes of form region
                    );
                    //
                    if (!frm.Initialized)
