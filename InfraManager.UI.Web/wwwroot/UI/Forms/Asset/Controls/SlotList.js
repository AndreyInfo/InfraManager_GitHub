define(['knockout', 'jquery', 'ajax', 'imList', 'models/Table/TableParts'], function (ko, $, ajaxLib, imLib, tpLib) {
        ...
        LinkList: function (ko_object, ajaxSelector, ko_canEdit) {
            var self = this;
            //
            self.ClassID = -1;//класс элементов списка
            ...
            self.HeaderText = ko.computed(function () {
                return getTextResource('Asset_SlotListHeader') + self.ItemsCount();
            });
            //
            ...
            {//splitter
                ...
                self.ResizeFunction = function (newHeight, isCustomCall) {
                    ...
                    else if (height <= 0 && delta < 0) {
                        var obj = self.Controller.GetAvailableHeightObj(self, false, true, true);
                        //
                        if (self.Height() === 0 && obj.Height() === 0) {
                            self.Controller.ResetDelta();
                            return;
                        }
                        //
                        var h = Math.abs(height);
                        //
                        var height2 = Math.max(0, obj.Height() + obj.Delta - h);
                        var dy = height2 - obj.Height();
                        //
                        obj.Height(height2);
                        obj.ResetHeightCoeff();
                        obj.Delta = h;
                        //
                        var nextItem = self.Controller.GetNextItem(self);
                        nextItem.Height(Math.max(0, nextItem.Height() - dy));
                        //
                        if (self.Height() !== 0) {
                            nextItem.Height(nextItem.Height() + self.Height());
                            nextItem.ResetHeightCoeff();
                            self.Height(0);
                        }
                    }
                };
            ...
            self.Init = function (idx, sortAsc) {
                ...
                require(['models/Table/Columns'], function (vm) {
                    $.when(userD).done(function (user) {
                        ...
                        var mod = new vm.ViewModel(tableModel, null, null, $('.slotList'));
                        ...
                        $.each(self._columnList, function (index, columnWithData) {
                            if (columnWithData && columnWithData.ColumnSettings && columnWithData.Data) {
                                ...
                                for (var i = 0; i < data.length; i++) {
                                    ...
                                    var rowInfo = self.GetOrCreateRow(i + '', null, false, null, null, data[i].Item);
                        ...
                        self.OnScroll = function () {
                            $('.slotList .tableHeader').css('margin-left', -this.scrollLeft);
                        };
                        //
                        $('.slotList .tabContent').bind('scroll', self.OnScroll);
                        //
                        retD.resolve();
                    });
                });
            ...
            var imListOptions = {};//параметры imList для списка 
            {
                ...
                imListOptions.LoadAction = function () {
                    ...
                    if (self.Data) {
                        require(['models/AssetForms/AssetForm.SlotReference'], function (slotLib) {
                            var retval = [];
                            self.columnWithDataList = self.Data.Columns;
                            ko.utils.arrayForEach(self.Data.Data, function (item) {
                                retval.push(new slotLib.SlotReference(self.imList, item));
                            });
