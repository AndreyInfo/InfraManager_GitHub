define(['jquery', 'knockout', 'formControl'], function ($, ko, fc) {
    var module = {
        formHelper: function (isSpinnerActive) {
            ...
            /**
             * @param { Element } obj - jQuery input
             * @param { String } searcherName - название искалки
             * @param { String } searcherTemplateName - название шаблона искалки
             * @param { Array } searcherParams - параметры конструктора для искалки
             * @param { Function } selectAction - function(objectInfo) действие выбора найденного объекта
             * @param { Function } resetAction - function() действие сброса значения
             * @param { Function } closeAction - function() действие закрытия искалки
             * @param { Promise } dLoad - deferred ожидание перед загрузкой
             * @param { Boolean } clearOnEmptyIgnore - игнорировать очищение при пустом поле ввода
             * @param { Array } searchItemsInit - массив поиска по существубщему списку
            */
            self.SetTextSearcherToField = function (
                obj,
                searcherName,
                searcherTemplateName,
                searcherParams,
                selectAction,
                resetAction,
                closeAction,
            ...
            self.ShowServiceCatalogueBrowser = function (mode, userID, selectedServiceID, typeCatalogue, serviceTypes) {
                ...
                require(['models/ServiceCatalogue/ServiceCatalogue'], function (vm) {
                    var mod = new vm.ServiceCatalogue(typeCatalogue, serviceTypes);

                    if (typeCatalogue === mod.TypesCatalogue.OnlyService) {
                        ctrl = new fc.control(
                            'serviceCatalogueBrowser',
                            'serviceCatalogueBrowser',
                            caption, true, true, true, 700, 550, buttons, cancelForm,
                            `data-bind="template: {name: \'ServiceCatalogue/Types/OnlyServices/ServiceCatalogueBrowserOnlyServices\'}"`
                        );

                        mod.OnServiceClick = function (item) {
                            const currentService = item.hasOwnProperty('Service') ? item.Service : item;
                            currentService.Name = `${currentService.ServiceCategory.Name} \\ ${currentService.Name}`; 
                            selectedItemAttendance = currentService;
                            ctrl.Close();
                        };
                    } else {
                        ctrl = new fc.control(
                            ...
                            'serviceCatalogueBrowser',
                            caption, true, true, true, 700, 550, buttons, cancelForm,
                            `data-bind="template: {name: \'ServiceCatalogue/ServiceCatalogueBrowser\'}"`
                        );
                    };
                    ...
                    mod.OnServiceItemAttendanceClick = function (serviceItemAttendance) {//override
                        if (!serviceItemAttendance || !serviceItemAttendance.IsAvailable)
                            return;
                                ...
                                click: function () {
                                    if (findClassIDs.length == 1) {
                                        ...
                                        if (findClassID === 701)
                                            ...
                                            mod.workorderList().OnSelectBtnClick(objectClassID);
                                        else if (findClassID === 371)
            ...
            self.SDEditorTemplateModes = {
                ...
                searcherEditForSubdivision: { template: 'SDForms/EditFieldTemplates/SelectSubdivision', name: 'objectSearcherForSubdivision' },
                textEdit: { template: 'SDForms/EditFieldTemplates/TextEditor', name: 'textEdit' },
                formulaEdit: { template: 'SDForms/EditFieldTemplates/FormulaEditor', name: 'formulaEdit' },
            ...
            self.ShowSDEditor = function (mode, options) {
                ...
                var ctrl = undefined;
                //
                const title = options.popupTitle || getTextResource('SDEditorCaption') + " '" + options.fieldFriendlyName + "'";
                ctrl = new fc.control('SDEditor', options.fieldName, title, true, true, true, 400, 500, buttons, null, '');
                if (!ctrl.Initialized)
                    return;
                ctrl.BeforeClose = function () {
                ...
                require(['models/SDForms/EditField'], function (vm) {
                    ...
                    mod.applyTemplate = function (specifiedModel) {
                        if (['Call.Initiator', 'Call.Client', 'Call.Owner', 'Call.Executor', 'Call.Accomplisher', 'WorkOrder.Initiator', 'WorkOrder.Assignor', 'WorkOrder.Executor', 'Problem.Owner',
                            ...
                            'SoftwareLicence.Utilizer', 'MassIncident.Owner'].indexOf(options.fieldName) != -1) {
            ...
            self.ShowClientInfoEditorForm = function (options, ClientObj) {
                $.when(operationIsGrantedD(71)).done(function (canViewUserProperties) {
                    if (canViewUserProperties) {
                        require(['models/ClientInfo/ClientEditForm'], function (EditForm) {
            ...
            //
            //Вызов формы допущенных лиц 
            self.ShowKBAUserList = function (kba, tmpKBAID) {
                if (isSpinnerActive != true)
                    showSpinner();
                //
                var buttons = {}
                var ctrl = undefined;
                var KBAUsers = undefined;

                ctrl = new fc.control('kbaUserListForm', 'kbaUserListForm', getTextResource('KBA_ListReadPersons'), true, true, true, 800, 500, buttons, null, 'data-bind="template: {name: \'../UI/Lists/KB/KBAAdmittedPersonsList\' }"');
                if (!ctrl.Initialized) {
                    hideSpinner();
                    require(['sweetAlert'], function () {
                        swal(getTextResource('OpenError'), getTextResource('CantDuplicateForm'), 'warning');
                ...
                //
                require(['../UI/Lists/KB/KBAAdmittedPersonsList'], function (vm) {
                    KBAUsers = new vm.ViewModel(kba, tmpKBAID);
                    //
                    KBAUsers.ForseClose = function () {
                        ctrl.Close;
                    };
                    //KBAUsers.Load();
                    //
                    ko.applyBindings(KBAUsers, document.getElementById(ctrl.GetRegionID()));
                    $.when(ctrlD, KBAUsers.LoadD).done(function (ctrlResult, loadResult) {
                        hideSpinner();
                    });
                });
            };

            //Вызов связанных статей
            self.ShowKBADependencyrList = function (kba) {
                if (isSpinnerActive != true)
                    showSpinner();
                //
                var buttons = [];
                var ctrl = undefined;
                var KBARelateds = undefined;
                ...
                ctrl = new fc.control('kbaUserListForm', 'kbaUserListForm', getTextResource('TooltipKB'), true, true, true, 800, 500, buttons, null, 'data-bind="template: {name: \'../UI/Lists/KB/KBARelatedArticleList\' }"');
                if (!ctrl.Initialized) {
                    hideSpinner();
                    require(['sweetAlert'], function () {
                        swal(getTextResource('OpenError'), getTextResource('CantDuplicateForm'), 'warning');
                ...
                //
                require(['../UI/Lists/KB/KBARelatedArticleList'], function (vm) {
                    KBARelateds = new vm.ViewModel(kba);
                    //
                    KBARelateds.ForseClose = function () {
                        ctrl.Close;
                    };
                    KBARelateds.Load();
                    ko.applyBindings(KBARelateds, document.getElementById(ctrl.GetRegionID()));
                    $.when(ctrlD, KBARelateds.LoadD).done(function (ctrlResult, loadResult) {
                        KBARelateds.UpdateAddButton();
                        hideSpinner();
                    });
                });
            };
            
            // Форма выбора исполнителя заявки/задания
            self.ShowExecutorSelector = function (obj) {
                if (isSpinnerActive !== true)
                    showSpinner();

                let ctrl = undefined;
                var ExecutorSelector = undefined;
                let buttons = [{
                    text: getTextResource('ButtonCancel'),
                    click: function () { ctrl.Close(); }
                }, {
                    ...
                    'Class': 'btnVisibility',
                    click: function () {
                        ExecutorSelector.Save();
                        ctrl.Close();
                    }
                }];

                ctrl = new fc.control('ExecutorSelectorForm', 'ExecutorSelectorForm', getTextResource('ExecutorChoice'),
                    true, true, true, 800, 500, buttons, null,
                    'data-bind="template: {name: \'../../UI/Forms/SD/ExecutorSelectorForm\' }"');
                ...
                let ctrlD = ctrl.Show();
                ctrl.ExtendSize(900, 400);
                ctrl.BeforeClose = function () { return true; };

                require([ '../../UI/Forms/SD/ExecutorSelectorForm' ], function (vm) {
                    ExecutorSelector = new vm.ViewModel(obj);
                    ExecutorSelector.ForceClose = function () { ctrl.Close; };
                    ExecutorSelector.ExecutorSelector(ExecutorSelector);
                    ExecutorSelector.Load();
                    ko.applyBindings(ExecutorSelector, document.getElementById(ctrl.GetRegionID()));
                    $.when(ExecutorSelector.loadD, ctrlD).done(function (ctrlResult, loadResult) {
                        ExecutorSelector.UpdateAddButton();
                        hideSpinner();
                    });
                });
            }

            // Форма выбора сервисов по категориям
            self.ShowSelectService = function (options) {
                if (!isSpinnerActive) {
                    showSpinner();
                };
                ...
                const buttonSelect = {
                    ...
                    click: function () {
                        if (options && options.onSave) {
                            options.onSave(model.getSelectedItems());
                        };
                ...
                const buttons = [buttonClose, buttonSelect];

                ctrl = new fc.control('SelectServiceForm', 'SelectServiceForm', getTextResource('ChoiceAdditionalServices'),
                    true, true, true, 488, 618, buttons, null,
                    'data-bind="template: {name: \'../UI/Forms/SD/SelectServiceForm/SelectServiceForm\', afterRender: afterRender}"', { maxWidth: 488, maxHeight: 618 });

                require(['../UI/Forms/SD/SelectServiceForm/SelectServiceForm'], function (vm) {
                    model = new vm.ViewModel(options.selected, options.excluded);
                    model.Load();

                    ko.applyBindings(model, document.getElementById(ctrl.GetRegionID()));

                    const form = $(`[aria-describedby="${ctrl.FormID}"]`);
                    const buttonsContainer = form.find('.ui-dialog-buttonpane');
