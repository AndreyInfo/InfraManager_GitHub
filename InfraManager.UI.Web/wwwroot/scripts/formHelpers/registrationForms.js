define(['jquery', 'knockout', 'formControl', 'usualForms', 'sdForms'], function ($, ko, fc, ufh, sfh) {
    var module = {
        formHelper: function (isSpinnerActive) {
                    ...
                    $.when(operationIsGrantedD(302)).done(function (workOrder_properties) {
                        ...
                        buttons[getTextResource('ButtonCreateWorkOrder')] = function () {
                            ...
                            $.when(d).done(function (result) {
                                ...
                                ctrl.Close();
                                retval.resolve(result.WorkOrderID);
                            });
                        };
                    ...
                    $.when(operationIsGrantedD(222)).done(function (problem_properties) {
                        ...
                        buttons[getTextResource('ButtonCreateProblem')] = function () {
                            ...
                            $.when(d).done(function (result) {
                                ...
                                ctrl.Close();
                                retval.resolve(result.ProblemID);
                            });
                        };
                        ...
                        if (problemData) {
                            ...
                            if (problemData.AddByMassIncident())
                                mod.MassIncidentId = problemData.MassIncidentId;
                            else if (problemData.AddByCallList())
                                mod.CallList = problemData.CallList;
                            else if (problemData.AddAs())
                                mod.AddAs = true;
                        }
                        //
                        ctrl.BeforeClose = function () {
            ...
            //
            // Форма регистрации массового инцидента
            self.ShowMassIncidentRegistration = function (options, miData) { // options - { calls: [ callID1, callID2, ... ] }
                ...
                require(['models/SDForms/MassIncidentRegistration'], function (vm) {
                    $.when(operationIsGrantedD(982)).done(function () { 
                        var buttons = {};

                        //
                        const ctrl = new fc.control(
                            'massIncidentRegistration', 'MassIncidentRegistration', getTextResource('MassIncidentRegistration'),
                            true, true, true, 760, 500, buttons, null,
                            'data-bind="template: {name: \'SDForms/MassIncidentRegistration\', afterRender: AfterRender}"'
                        );

                        if (!ctrl.Initialized)
                            return;
                        //
                        const oldSizeChanged = ctrl.SizeChanged;
                        ctrl.SizeChanged = function () {
                            oldSizeChanged();
                        };
                        //
                        ...
                        const viewModel = new vm.ViewModel(options, ctrl.GetRegionID());
                        if(miData) {
                           viewModel.miData = miData;
                           if(miData.AddAs())
                               viewModel.AddAs = true;
                        }
                        
                        buttons[getTextResource('MassIncident_CreateNew')] = function () {
                            var d = viewModel.ValidateAndSave();
                            $.when(d).done(function (result) {
                                if (result == null)
                                    return;
                                //                                
                                ...
                                retval.resolve(result.ID);
                                require(['sweetAlert'], function () {
                                    swal(getTextResource('MassIncidentRegisteredMessage').replace('{0}', result.ID));
                                });
                            });
                        };
                        ...
                        $.when(ctrlD, viewModel.Load()).done(function () {     
                            const elem = document.getElementById(viewModel.FormID);
                            try {
                        ...
                        $.when(ctrlD, viewModel.renderedD).done(function () {
                            if (!ko.components.isRegistered('massIncidentRegistrationFormCaptionComponent'))
                                ko.components.register('massIncidentRegistrationFormCaptionComponent', {
                                    template: '<span class="color-point" style="margin-right:5px;" data-bind="style: { background: $priorityColor}" /><span data-bind="text: $captionText"/>'
                                });
                            ctrl.BindCaption(viewModel, "component: {name: 'massIncidentRegistrationFormCaptionComponent', params: { $priorityColor: PriorityColor, $captionText: getTextResource(\'MassIncident_RegistrationFormCaption\')} }");
                            //
                            hideSpinner();
                        });
                    });
