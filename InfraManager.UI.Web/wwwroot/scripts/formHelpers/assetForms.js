define(['jquery', 'knockout', 'formControl', 'usualForms'], function ($, ko, fc, ufh) {
    var module = {
        formHelper: function (isSpinnerActive) {
            ...
            //
            self.ShowAssetModelSelect = function (objectInfo, callbackFunc) {
                if (isSpinnerActive != true)
                    showSpinner();
                //
                var mod = null;
                ...
                //
                var name = 'assetModelLinkForm';
                var formName = objectInfo.Caption ? objectInfo.Caption : getTextResource('AssetModelLinkHeader');
                ctrl = new fc.control(name, name, formName, true, true, true, 800, 500, buttons, afterClose, 'data-bind="template: {name: \'AssetForms/AssetModelLink/MainForm\', afterRender: afterRender}"');
                if (!ctrl.Initialized)
                    return;
                //
                ...
                require(['models/AssetForms/AssetModelLink/AssetModelLink'], function (vm) {
                    var mainRegionID = ctrl.GetRegionID();
                    var $region = $('#' + mainRegionID);
                    mod = new vm.ViewModel($region, objectInfo.activeRequestMode, objectInfo.hasLifeCycleOnly, objectInfo.modelSearchMode, objectInfo.productCatalogueItemInfo);
                    //
                    mod.isSelectButtonVisible.subscribe(function (newValue) {
                        if (newValue == false && buttons.length != 1) {
                            buttons = [bClose];
                        ...
                        else if (newValue == true && ((buttons.length == 1 && objectInfo.SelectOnlyOne === true) || (buttons.length >= 1 && objectInfo.SelectOnlyOne === false))) {
                            buttons = [bClose, bSelect];
                            ctrl.UpdateButtons(buttons);
                        }
                    });
            ...
            //
            self.ShowSoftwareLicenceSchemeLink = function (
                activeRequestMode,
                hasLifeCycleOnly,
                callbackFunc,
                modelSearchMode,
                ...
                //
                var name = 'SchemeLinkForm';
                var formName = formCaption ? formCaption : getTextResource('SoftwareLicenceScheme_Select');
                ctrl = new fc.control(name, name, formName, true, true, true, 1000, 760, buttons, afterClose, 'data-bind="template: {name: \'SoftwareLicence/SoftwareLicenceSchemeLink/MainForm\', afterRender: afterRender}"');
                if (!ctrl.Initialized)
                    return;
                //
                var ctrlD = ctrl.Show();
                ctrl.ExtendSize(1000, 760);
                //
                require(['models/AssetForms/SoftwareLicenceSchemeLink'], function (vm) {
                    var mainRegionID = ctrl.GetRegionID();
                    var $region = $('#' + mainRegionID);
                    mod = new vm.ViewModel($region, activeRequestMode, hasLifeCycleOnly, modelSearchMode, productCatalogueItemInfo, showSchemeObject);
                    //
                    mod.isSelectButtonVisible.subscribe(function (newValue) {
                        if (newValue == false && buttons.length != 1) {
                            buttons = [bClose];
            ...
            //вызов формы схемы лицензирования
            self.ShowLicenceSchemeForm = function (id) {
                require(['ui_forms/Settings/LicenceSchemes/frmLicenceScheme'], function (jsModule) {
                    jsModule.ShowDialog(id, false, isSpinnerActive);
                });
            };
            //
            ...
            //вызов формы Список инсталляций
            self.ShowSoftwareInstallationObjectForm = function (id) {
                if (isSpinnerActive) {
                    hideSpinner();
                }
                //вызов формы Инсталляции
                require(['ui_forms/Asset/SoftwareInstallation/frmSoftwareInstallation'], function (jsModule) {
                    jsModule.ShowDialog(id, false, true);
                });
            };
            //вызов формы 
            self.ShowCommercialSoftwareModelForm = function (id) {
                require(['ui_forms/Asset/Models/frmSoftwareCommercialModel'], function (jsModule) {
                    jsModule.ShowDialog(id, isSpinnerActive);
                });
            };
        }
