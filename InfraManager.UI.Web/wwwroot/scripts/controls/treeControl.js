define(['knockout', 'jquery', 'ajax'], function (ko, $, ajaxLib) {
    var module = {
        control: function (template) {
            ...
            self.AvailableTemplateID = ko.observable(null);
            ...
            self.Load = function () {
                ...
                var data = {
                    ...
                    AvailableTemplateID: self.AvailableTemplateID() ? self.AvailableTemplateID() : null,
                        ...
                        if (newVal && newVal.Result === 0) {
                            ...
                            if (list) {
                                ko.utils.arrayForEach(list, function (el) {
                                    if (self.InitialCheckedNodes && self.InitialCheckedNodes.length > 0) {
                                        //el.CheckedState = (self.InitialCheckedNodes.indexOf(el.ID) != -1) ? module.checkedStates.Checked : module.checkedStates.Unchecked;
                                        el.Checked = (self.InitialCheckedNodes.indexOf(el.ID) != -1);
                                    }
                        ...
                        if (newVal && newVal.Result === 0) {
                            ...
                            if (list) {
                                ...
                                if (list.length == 0)
                                    ...
                                    else if (node.CheckedState() == module.checkedStates.Checked)
                                        el.Checked = true;
                                    //
            ...
            //
            self.OpenToNode = function (nodeID, nodeClassID) {
                var retD = $.Deferred();
                //
                ...
                var recursiveGetter = function (nodeCollection) {
                    ...
                    ko.utils.arrayForEach(nodeCollection, function (node) {
                        if (node) {
                            ...
                            //
                            if (node.Nodes && node.Nodes() && (node.CheckedState() !== module.checkedStates.Unchecked || self.OnlyTwoState()))
            ...
            //
            self.CheckAllNodes = function (nodes) {
                if (!self.ShowCheckboxes())
                    return null;
                //
                ko.utils.arrayForEach(self.firstNodes(), function (node) {
                    if (node && node.CheckedState() != module.checkedStates.Checked)
                        node.CheckedState(module.checkedStates.Checked); //recursive go down
                });
            };
            //
            self.defaultConfig = {
                ...
                AvailableTemplateID: null, //ProductCatalogTemplate, TemplateID, используется в навигаторе по каталогу продуктов. если задано, то доступны лишь типы с Шаблоном
                ...
                //
                InitalSelectionNodes: []   // список отмеченных нодов при загрузке. Используется когда при загрузке новой ветки нужно проставить галочки. Используется только в режиме ShowCheckboxes=true && OnlyTwoState=true
            };
            //
            self.init = function ($region, type, settings) {
                ...
                self.AvailableTemplateID(config.AvailableTemplateID);
                ...
                //
                self.InitialCheckedNodes = config.InitialCheckedNodes;
                //
            ...
            self.ShowInNewWindow = function (windowCaption) {
                ...
                require(['usualForms'], function (fhMod) {
                    ...
                    var options = {
                        ...
                        AvailableTemplateID: self.AvailableTemplateID(),
                        HasLifeCycle: self.HasLifeCycle(),
