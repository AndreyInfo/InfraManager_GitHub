define(['jquery', 'ajax', 'knockout', 'usualForms', 'jqueryUI', 'jqueryMouseWheel'], function ($, ajaxLib, ko, formHelperLib) {
    var module = {
         ...
         * @param { Array } searchItemsInit - массив поиска по существующему списку
         */
        control: function (
            obj,
            searcherName,
            ...
            //
            var self = this;
            self.SearcherParams = searcherParams;
            ...
            self.searchPrivate = function (searchText) {
                ...
                if (searchItemsInit && Array.isArray(searchItemsInit)) {
                    let selectedItemFound = false;
                    self.Items.removeAll();
                    self.PreservedItems.removeAll();

                    searchItemsInit.forEach(function (item) {
                        if (!item.FullName.toLowerCase().includes(searchText.toLowerCase()))
                            return;

                        const tmp = new createItem(item);
                        self.Items().push(tmp);
                        self.PreservedItems().push(tmp);
                        
                        if (self.SelectedItem() && self.SelectedItem().ID == tmp.ID) {
                            self.SelectedItem(tmp);                       
                ...
                var url = '/api/searchrequests/' + searcherName;
                var searchCriteria = {
                    Text: searchText.trim(),
                    UserId: self.CurrentUserID
                };

                searchCriteria = Object.assign(searchCriteria, self.SearcherParams);

                // Note: Backend expects the following scheme for ServiceItemAndAttendanceSearchCriteria.
                if (searcherName == 'ServiceItemAndAttendanceSearcher') {
                    searchCriteria.CallTypeId = searchCriteria['0'];
                }

                var data = JSON.stringify({ Content: JSON.stringify(searchCriteria) });
