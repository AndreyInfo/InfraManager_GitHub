    ...
    var module = {
        control: function ($region, clearAction, focusSearcher) {//к чему будем байндить; метод очистки поиска; сфокусировать text input
            ...
            self.InitCategories = function () {
                ...
                self.categories.push(new Category(823, getTextResource('MassiveIncidents'), 'icon-extended-search__mass_incident', true));
            ...
            self.GetFriendlyViewName = function (viewName) {
                ...
                if (viewName == 'Hardware')
                    retval += getTextResource('List_Hardwares');
                if (viewName == 'AllMassIncidentsList')
                    retval += getTextResource('List_AllMassiveIncidents');
                return retval + '"';
            }
            //
