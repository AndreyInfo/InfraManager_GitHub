define(['knockout', 'jquery', 'ajax', 'usualForms', 'ttControl', 'models/SDForms/SDForm.CallReferenceList', 'models/SDForms/SDForm.ProblemReferenceList', 'models/SDForms/SDForm.WOReferenceList', 'models/SDForms/SDForm.KBAReferenceList', 'models/SDForms/SDForm.MassiveIncidentList'], function (ko, $, ajaxLib, fhModule, tclib, callReferenceListLib, problemReferenceListLib, WOReferenceListLib, kbaReferenceList, massIncidentListLib) {
    var module = {
        ViewModel: function ($region, openedModule, formCtrl) {
            ...
            self.massiveIncidentList = ko.observable(null);
            //
            self.Search = function (phrase) {
                ...
                var _massiveIncidentList = new massIncidentListLib.MassiveIncidentList(null, null, null, self.ReadOnly, self.CanEdit);
                //
                self.problemList(_problemList);
                self.callList(_callList);
                self.workorderList(_workorderList);
                self.kbaList(_kbaList);
                self.massiveIncidentList(_massiveIncidentList);
                        ...
                        if (response) {
                            if (response.length > 0) {
                                ...
                                response.forEach(function (el) {
                                    ...
                                    else if (el.ClassID == 823) {
                                        _massiveIncidentList.PushData(el.FoundObjectList);
                                        self.massiveIncidentList(_massiveIncidentList);
                                    }
                                });
