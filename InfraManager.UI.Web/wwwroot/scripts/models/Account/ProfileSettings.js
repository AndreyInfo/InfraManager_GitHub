        ...
        ViewModel: function (userID) {
            ...
            self.Load = function () {
                ...
                ajaxControl.Ajax(null,
                    ...
                    function (response) {
                        ...
                        //
                        self.ListView_CompactMode(self.setting.ListView.CompactMode);
                        self.ListView_GridLines(self.setting.ListView.GridLines);
                        self.ListView_Multicolor(self.setting.ListView.Multicolor);
                    });
                //              
                ...
                self.ajaxControl_save.Ajax($('#profile'),
                    {
                        ...
                        dataType: 'text',
                        method: 'POST',
                        data: JSON.stringify(self.setting)
                    },
                    function () {
                        if (reloadPage == true) {
                            showSpinner();
            ...
            {//listView settings
                ...
                self.ListView_CompactMode.subscribe(function (newValue) {
                    if (self.setting != null) {
                        if (self.setting.ListView.CompactMode == newValue)
                            return;
                        self.setting.ListView.CompactMode = newValue;
                        self.Save(false);
                    }
                });
                //
                ...
                self.ListView_GridLines.subscribe(function (newValue) {
                    if (self.setting != null) {
                        if (self.setting.ListView.GridLines == newValue)
                            return;
                        self.setting.ListView.GridLines = newValue;
                        self.Save(false);
                    }
                });
                //
                ...
                self.ListView_Multicolor.subscribe(function (newValue) {
                    if (self.setting != null) {
                        if (self.setting.ListView.Multicolor == newValue)
                            return;
                        self.setting.ListView.Multicolor = newValue;
                        self.Save(false);
                    }
                });
            }
            ...
            //
            {
                ...
                self.LoadDeputy = function () {
                    require(['models/Account/DeputyTable'], function (vm) {
                        var mod = new vm.ViewModel(self.curUser());
                        self.Deputy(mod);
                        mod.Load();
                    });
                }
