define(['knockout', 'jquery', 'ajax', 'ttControl'], function (ko, $, ajaxLib, tclib) {
    var module = {
        ViewModel: function () {
            ...
            var guantt = { Name: "График проведения изменений", IsGuantt: true };
                            ...
                            if (onLoadFunc) {
                                ...
                                self.$iframe.on('load', function () {
                                    try {
                                        ...
                                        if (_document.forms.length > 0) {
                                            _document.forms[0].style.height = '100%';
                                        }
                                        //
                                        var dxWinPlatforms = _document.getElementsByClassName('dxWindowsPlatform');
                                        if (dxWinPlatforms.length > 0) {
                                            var container = [0];//'dxWindowsPlatform' - на chrome, win10. проверить для других браузеров.
                                        ...
                                        _document.showObjectForm = function (objectClassID, objectID) {
                                            if (objectClassID == 701 // IMSystem.Global.OBJ_CALL
                                                || objectClassID == 119 // IMSystem.Global.OBJ_WORKORDER
                                                || objectClassID == 702 // IMSystem.Global.OBJ_PROBLEM
                                                || objectClassID == 703 // IMSystem.Global.OBJ_RFC
                                                || objectClassID == 823 // IMSystem.Global.OBJ_MassIncident
                                            ) {
        ...
        GuanttLoad: function () {
            ...
            if (self.$iframe.length > 0) {
                ...
                var url = host + '/UI/Lists/RFCGantt/index.html';
