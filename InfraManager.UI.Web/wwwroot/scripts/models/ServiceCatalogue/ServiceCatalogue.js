    ...
    var module = {
        ServiceCategory: function (serviceCatalogue, obj) {
            ...
            self.AvailableServiceList = ko.computed(function () {
                return self.ServiceList().filter(el => el.IsAvailable);
            });
            self.VisibleServiceList = ko.computed(function () {
                return serviceCatalogue.FilterBySLA() === true 
                    ? self.AvailableServiceList() 
                    : self.ServiceList();
            });
        },
        ...
        Service: function (serviceCatalogue, serviceCategory, obj) {
            ...
            //
            self.ID = obj.ID;
            self.PrefixName = function (index) {
                if (index < 10) {
                    return `${0}${index}. ${self.Name}`;
                } else {
        ...
        CallTypeInfo: function (obj) {
            ...
            self.ID = obj.ID;
            self.Name = obj.Name;
            self.ImageSource = '/api/calltypes/image/' + obj.ID;
        ...
        CallInfo: function (obj) {
            ...
            self.CallSummaryName = obj.Summary;
            self.ServiceItemAttendanceFullName = obj.ServiceItemOrAttendance;
            self.UtcDateRegistered = parseDate(Date.parse(obj.UtcDateRegistered));
        ...
        ServiceCatalogue: function (typeCatalogue, serviceTypes) {
            var self = this;
            //
            ...
            self.CurrentTypeCatalogue = typeCatalogue;

            self.IsOnlyService = function () {
                return self.CurrentTypeCatalogue === self.TypesCatalogue.OnlyService;
            };
            ...
            self.VisibleCallInfoList = ko.computed(function () {
                ...
                let retval = [];
                //
                for (let i = 0; i < Math.min(10, self.CallInfoList().length) ; i++)
            ...
            self.OnChangeFavoritesStateClick = function (serviceItemAttendance) {
                if (!serviceItemAttendance)
                    return;
                //
                const oldValue = serviceItemAttendance.IsInFavorite();
                const data = {
                    UnderControl: !oldValue
                };
                var resource = serviceItemAttendance.ClassID == 406 ? 'serviceItems/' : 'serviceAttendances/';
                ...
                ajaxCtrl.Ajax(self.$Region,
                    {
                        ...
                        url: '/api/' + resource + serviceItemAttendance.ID + '/customControls/my/'
                    },
                    function (details) {
            ...
            self.OnCreateCallAsClick = function (callInfo) {
                ...
                {
                    method: 'GET',
                    url: `/api/calls/${callInfo.ID}`,
                },
                function (newVal) {
                    if (!!newVal) {
                        const objectInfo = newVal;
                        if (objectInfo && objectInfo.ID) {
                            require(['registrationForms', 'models/SDForms/CallForm.Call'], function (lib, callLib) {
                                const fh = new lib.formHelper(true);
                                objectInfo.ImageSource = callInfo.ImageSource;
                                $.when(userD).done(function (user) {
                                    objectInfo.ClientID = user.UserID;
                                });
                                const currentViewMode = self.SelectedViewMode();
                                self.SelectedViewMode(module.Modes.none);
                                $.when(fh.ShowCallRegistrationLiteFullScreen(null, null, null, new callLib.Call(self, objectInfo), function () {
                                    self.SelectedViewMode(currentViewMode);
                                })).done(self.SetterForCloser);
                            });
                        }
            ...
            {//все по поиску
                ...
                var searchResultItem = function (service, uniqueResults, totalResults, findedPlaces, findedAttendance, findedItems) {
                    var selfSeacrh = this;
                    selfSeacrh.Service = service;
                    selfSeacrh.Unique = uniqueResults.length;
                    selfSeacrh.Total = totalResults;
                    selfSeacrh.FindedPlaces = findedPlaces;
                    selfSeacrh.FindedAttendance = findedAttendance;
                    selfSeacrh.FindedItems = findedItems;
                    selfSeacrh.NeedFilrerSIA = (selfSeacrh.FindedPlaces.length > 0 && selfSeacrh.FindedPlaces.indexOf(findedPlacesTypes.ServiceName) == -1 && selfSeacrh.FindedPlaces.indexOf(findedPlacesTypes.ServiceNote) == -1);
                    selfSeacrh.NeedShow = function (id) {
                        return (selfSeacrh.FindedAttendance.indexOf(id) != -1 || selfSeacrh.FindedItems.indexOf(id) != -1);
                    }; //только для фильтрации по предыдущему пункту
                    //
                    if (self.IsOnlyService()) {
                        selfSeacrh.NeedShowServiceNote = "";
                    } else {
                        selfSeacrh.NeedShowServiceNote = selfSeacrh.FindedPlaces.indexOf(findedPlacesTypes.ServiceNote) != -1;
                    };

                    selfSeacrh.NeedShowSIANote = function (id) {
                        return (selfSeacrh.FindedPlaces.indexOf(findedPlacesTypes.SIANote) != -1) && selfSeacrh.NeedShow(id);
                    };

                    selfSeacrh.NeedShowItemsBlock = ko.computed(function () {
                        if (self.IsOnlyService()) {
                            return false;
                        };

                        return (selfSeacrh.NeedFilrerSIA == false && selfSeacrh.Service.VisibleServiceItemList().length > 0) || (selfSeacrh.NeedFilrerSIA == true && selfSeacrh.FindedItems.length > 0);
                    });

                    selfSeacrh.NeedShowAttendanceBlock = ko.computed(function () {
                        if (self.IsOnlyService()) {
                            return false;
                        };

                        return (selfSeacrh.NeedFilrerSIA == false && selfSeacrh.Service.VisibleServiceAttendanceList().length > 0) || (selfSeacrh.NeedFilrerSIA == true && selfSeacrh.FindedAttendance.length > 0);
                    });
                };
                //
            ...
            {//контрол дерева сервисов
                ...
                self.OnServiceClick = function (service) {
                    ...
                    loadServiceDetails(service, function (service) {
                        self.SelectedObject(service);
                        self.CollapseExpandService(service);
                    });

                    function loadServiceDetails(service, callback) {
                        const ajaxCtrl = new ajaxLib.control();
                        ajaxCtrl.Ajax(self.$Region,
                            {
                                dataType: 'json',
                                contentType: 'application/json',
                                method: 'GET',
                                url: '/bff/serviceCategories/' + service.ServiceCategory.ID + '/' + service.ID,
                                data: { userID: self.CurrentUserID, types: self.ServiceTypes, },
                                traditional: true,
                            },
                            function (response) {
                                try {
                                    let item = response[0].ServiceList[0];
                                    service.Name = item.Name;
                                    service.Note = item.Note;
                                    service.IsNoteEmpty = item.Note == '' || item.Note == null || item.Note == undefined;
                                    service.IsAvailable = item.IsAvailable;
                                }
                                catch (error) { }
                                callback(service);
                            },
                ...
                self.OnServiceItemAttendanceClick = function (serviceItemAttendance) {
                    ...
                    loadSeviceItemDetails(serviceItemAttendance, function (serviceItemAttendance) { 
                    ...
                    function loadSeviceItemDetails(serviceItemAttendance, callback) {
                        const ajaxCtrl = new ajaxLib.control();
                        ajaxCtrl.Ajax(self.$Region,
                            {
                                dataType: 'json',
                                contentType: 'application/json',
                                method: 'GET',
                                url: '/bff/serviceCategories/' + serviceItemAttendance.Service.ServiceCategory.ID + '/' + serviceItemAttendance.Service.ID + '/' + serviceItemAttendance.ID,
                                data: { userID: self.CurrentUserID, types: self.ServiceTypes, },
                                traditional: true,
                            },
                            function (response) {
                                try {
                                    let item = response[0].ServiceList[0].ServiceItemAttendanceList[0];
                                    serviceItemAttendance.ClassID = item.ClassID;
                                    serviceItemAttendance.Name = item.Name;
                                    serviceItemAttendance.Note = item.Note;
                                    serviceItemAttendance.IsNoteEmpty = item.Note == '' || item.Note == null || item.Note == undefined;
                                    serviceItemAttendance.IsAvailable = item.IsAvailable;
                                    serviceItemAttendance.Parameter = item.Parameter;
                                    serviceItemAttendance.Summary = item.Summary;
                                }
                                catch (error) { }
                                callback(serviceItemAttendance);
                            },
                            function () {
                                callback(serviceItemAttendance);
                            });
            ...
            {//контрол вкладок                
                ...
                self.SelectedViewMode.subscribe(function (newValue) {
                    if (newValue == module.Modes.serviceCatalogue || newValue == module.Modes.serviceCatalogueBySLA) {
                        let setMainViewTemplateName = 'ServiceCatalogue';

                        if (self.IsOnlyService()) {
                            setMainViewTemplateName = 'Types/OnlyServices/ServiceCatalogueOnlyServices';
                        };

                        self.ViewTemplateName(`ServiceCatalogue/${setMainViewTemplateName}`);
                    ...
                    function (response) {
                        if (response) {
                            ...
                            $.each(response, function (index, callTypeInfo) {
                                const cti = new module.CallTypeInfo(callTypeInfo);
            ...
            self.LoadCallInfoList = function () {
                ...
                const requestInfo = {
                    ...
                    CustomFilters: [
                        {
                            PropertyName: "ClientFullName",
                            Type: 5, // SelectorMultiple
                            SearchOperation: 0, // Equal
                            SelectedValues: [{ ID: self.CurrentUserID }]
                        }
                    ]
                };
                ...
                ajaxControl.Ajax(self.$Region.find('.sc_favoritesLast'),
                    {
                        method: 'POST',
                        url: '/api/calls/reports/allcalls',
                    ...
                    function (response) {
                        if (response) {
                            ...
                            const callInfoList = response
                                .map(el => new module.CallInfo(el))
                                .sort(function (a, b) {
                                    if (a.UtcDateRegistered < b.UtcDateRegistered) {
                                        return -1;
                                    }
                                    if (a.UtcDateRegistered > b.UtcDateRegistered) {
                                        return 1;
                            ...
                            self.CallInfoList(callInfoList);
            ...
            self.SelectedItemPath = ko.computed(function () {
                ...
                if (self.IsOnlyService()) {
                    const index = self.VisibleServiceCategoryList().findIndex(function (cat) {
