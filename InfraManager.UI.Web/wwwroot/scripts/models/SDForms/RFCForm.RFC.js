define(['knockout', 'jquery', 'models/SDForms/SDForm.User'], function (ko, $, userLib) {
    var module = {
        RFC: function (parentSelf, rfcData) {
            var rfcself = this;
            var self = parentSelf;
            if (!rfcData.EntityStateID && !rfcData.WorkflowSchemeID && rfcData.EntityStateName) {
            ...
            rfcself.UtcDateDetectedJS = rfcData.UtcDateDetected;
            ...
            rfcself.UtcDateClosedJS = rfcData.UtcDateClosed;
            ...
            rfcself.UtcDateSolvedJS = rfcData.UtcDateSolved;
            ...
            rfcself.UtcDatePromisedJS = rfcData.UtcDatePromised;
            if (rfcData.UtcDatePromised)
            ...
            rfcself.UtcDateStartedJS = rfcData.UtcDateStarted;
            if (rfcData.UtcDateStarted)
            ...
            //
            if (rfcData.ManHours)
                rfcself.ManhoursInMinutes = ko.observable(rfcData.ManHours)
            else rfcself.ManhoursInMinutes = ko.observable(0);

            if (rfcData.ManhoursNorm)
                rfcself.ManhoursNorm = ko.observable(rfcData.ManhoursNorm)
            else rfcself.ManhoursNorm = ko.observable(0);

            if (rfcData.ManhoursString)
                rfcself.ManHours = ko.observable(rfcData.ManhoursString)
            else rfcself.ManHours = ko.observable('');

            if (rfcData.ManhoursNormString)
                rfcself.ManhoursNormString = ko.observable(rfcData.ManhoursNormString)
            else rfcself.ManhoursNormString = ko.observable('');
            //
            ...
            var uri = '/api/changerequests/' + rfcData.ID;
            rfcself.getCalls = function ($ctrl) {
                ...
                require(['models/SDForms/Shared/callReferencesFetcher'], function (callRefs) {
                    $.when(callRefs.GetProblemCalls(uri, $ctrl)).done(function (result) {
                        if (result.success) {
                            rfcself.CallCount(result.ids.length);
                        }
            ...
            rfcself.addCalls = function (ids, onAdd, $ctrl) {
                var retD = $.Deferred();
                var processed = 0;
                ...
                require(['models/SDForms/Shared/callReferencesFetcher'], function (callRefs) {
                    callRefs.ReferenceMultipleCallsWithProblemAsync(
                        ...
                        function () {
                            rfcself.CallCount(rfcself.CallCount() + processed);
            ...
            rfcself.deleteCalls = function (ids, onDelete, $ctrl) {
                var retD = $.Deferred();
                var processed = 0;
