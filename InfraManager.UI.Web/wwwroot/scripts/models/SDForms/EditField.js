define(['knockout', 'jquery', 'ajax', 'usualForms', 'dateTimeControl', 'fingerPrintLib', 'models/SDForms/SDForm.User', 'jqueryStepper']
    , function (ko, $, ajaxLib, fhmodel, dtLib, fingerPrint, userLib, jqueryStepper) {
    var module = {
        ...
        ViewModel: function (mode, options) {
            ...
            self.createComboBoxValue = function (enumValue) {
                var thisObj = this;
                //
                thisObj.ID = enumValue.ID ?? enumValue.Id;
                    ...
                    self.ajaxControl_comboBoxValue.Ajax(self.$region.find('.combobox'),
                        ...
                        function (response) {
                            if (response) {
                                ...
                                //
                                let responseComboBoxList = response;
                                if (!!options.comboBoxOptionsHandler) {
                                    responseComboBoxList = options.comboBoxOptionsHandler(response);
                                }
                                //
                                $.each(responseComboBoxList, function (index, simpleEnum) {
                                ...
                                $.when(self.TemplateLoadedD).done(function () {
                                    ...
                                    if (options.comboBoxExpanded) {
                                        const valueList = self.comboBoxValueList();
                                        if (valueList.length) {
                                            self.comboBoxSelectedValue(valueList[0]);
                                        }
                                        self.$region.find('.combobox').find('.control').click();
                                    }
                                });
                                //
            ...
            self.withQueue = ko.observable(false);
                    ...
                    self.ajaxControl_loadQueueInfo.Ajax(self.$region.find('.edit-field-user-input-details'),
                        {
                            dataType: "json",
                            method: 'GET',
                            url: `/api/groups/${self.selectedObjectInfoQueue.ID}`,
                            headers: { 'x-device-fingerprint': self.fingerprintJs.fHash }
                        },
                        function (response) {
                            if (response) {
                                self.objectSearcherControl.SetSearchParameters({QueueId: response.ID});
                    ...
                    self.ajaxControl_loadUserInfo.Ajax(self.$region.find('.edit-field-user-input-details'),
                        {
                            dataType: "json",
                            method: 'GET',
                            url: '/api/users/' + self.selectedObjectInfo.ID,
                            headers: { 'x-device-fingerprint': self.fingerprintJs.fHash }
                        },
                        function (response) {
                            if (response)
                    ...
                    self.ajaxControl_loadQueueInfo.Ajax(self.$region.find('.edit-field-user-input-details'),
                        {
                            dataType: "json",
                            method: 'GET',
                            url: '/api/groups/' + self.selectedObjectInfo.ID,
                            headers: { 'x-device-fingerprint': self.fingerprintJs.fHash }
                        },
                        function (response) {
                            if (response) {
                                var options = {
                                    UserID: response.ResponsibleUserID,
                                    UserType: userLib.UserTypes.withoutType,
                                ...
                                response.QueueTypeName = ko.observable(response.QueueTypeName);
            ...
            self.formulaControlD = $.Deferred();
            self.FormulaEditor = function () {

                self.parameterModel = options.model;
                self.applyTemplate(self);               

                self.formulaControlD.resolve();
                return self.formulaControlD.promise();
            };
            ...
            self.CreateNumberEditor = function () {
                ...
                $.when(self.TemplateLoadedD).done(function () {
                    ...
                    $input.stepper({
                        ...
                        wheelStep: self.options.wheelStep || 1,
                        arrowStep: self.options.arrowStep || 1,
            ...
            self.Load = function () {
                ...
                else if (mode.name == fh.SDEditorTemplateModes.searcherEditForSubdivision.name)
                    $.when(self.CreateSearcherEditor()).done(tmp);
                ...
                else if (mode.name == fh.SDEditorTemplateModes.formulaEdit.name)
                    $.when(self.FormulaEditor()).done(tmp);
            ...
            self.Focus = function () {
                ...
                else if (mode.name == fh.SDEditorTemplateModes.searcherEdit.name || mode.name == fh.SDEditorTemplateModes.searcherEditForSubdivision.name)
            ...
            self.Save = function (isReplaceAnyway) {
                ...
                $.when(self.TemplateLoadedD).done(function () {
                    ...
                    if (mode.name == fh.SDEditorTemplateModes.richEdit.name) {
                        ...
                        $.when(self.objectSearcherControlD).done(function () {
                            objectToReturn = { User: self.selectedObjectInfo, Queue: self.selectedObjectInfoQueue };
                            //
                            d.resolve(true);
                        });
                    }
                        ...
                        $.when(self.objectSearcherControlD).done(function () {
                            objectToReturn = self.selectedObjectInfo;
                            objectToReturn.WithQueue = self.withQueue();
                            if (self.selectedUser() != null)
                                objectToReturn['UserInfo'] = self.selectedUser();
                            if (self.selectedObjectFullName() && self.selectedObjectFullName().length > 0 && options.allowAnyText == true &&
                                (!objectToReturn || objectToReturn && objectToReturn.FullName != self.selectedObjectFullName()))
                                objectToReturn = { ID: null, ClassID: null, FullName: self.selectedObjectFullName(), Details: '', 'WithQueue': self.withQueue() };//objectInfo
                            //
                            data.NewValue = objectToReturn == null ? null : JSON.stringify({ 'id': objectToReturn.ID, 'fullName': self.selectedObjectFullName(), 'WithQueue': self.withQueue() });
                            data.OldValue = self.options.oldValue == null ? null : JSON.stringify({ 'id': self.options.oldValue.ID, 'fullName': self.options.oldValue.FullName, 'classID': self.options.oldValue.ClassID, 'WithQueue': self.withQueue() });
                            data.Params = objectToReturn == null ? [] : ['' + objectToReturn.ClassID, objectToReturn.FullName];
                            //
                            d.resolve(true);
                        });
                        ...
                        $.when(self.htmlControlD).done(function () {
                            objectToReturn = self.htmlControl.IsEmpty() ? '' : self.htmlControl.GetHTML();
                        ...
                        $.when(self.formulaControlD).done(function () {
                            objectToReturn = self.$region.find('.textResult').val();
                            //
                            data.NewValue = JSON.stringify({ 'text': objectToReturn });
                            data.OldValue = JSON.stringify({ 'text': options.oldValue });
                            //
                        ...
                        $.when(self.datetimePickerControlD).done(function () {
                            ...
                            if (options.allowNull) {
                                ...
                                if (options.allowTime) {
                                    objectToReturn = dtLib.Date2String(dv, false) == dtText ? dtLib.GetMillisecondsSince1970(dv) : '';
                                } else {
                    ...
                    $.when(d).done(function (result) {
                        ...
                        if (options.nosave == true) {
                            ...
                            $.when(options.save(data))
                                .done(function (success) { retD.resolve(success); });
                        } else {
                            self.ajaxControl_save.Ajax(
                                null,//self.$region, two spinner problem
                                {
                                    ...
                                    url: options.urlSetField ? options.urlSetField : '/sdApi/SetField',
                                    data: options.GetData ? options.GetData(data.NewValue) : data,
                                    contentType: "application/json",
                                    headers: {
                                        'x-device-fingerprint': self.fingerprintJs.fHash
                                    }
                                },
                                function (retModel) {
                                    if (options.callback) {
                                        if (options.callback(retModel)) {
                                            if (options.onSave != null)
                                                options.onSave(objectToReturn);
                                            //
