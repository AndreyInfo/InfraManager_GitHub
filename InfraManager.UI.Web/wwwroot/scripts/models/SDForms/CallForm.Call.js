define(['ajax', 'knockout', 'jquery', 'models/SDForms/SDForm.User'], function (ajaxLib, ko, $, userLib) {
    var module = {
        Call: function (parentSelf, callData) {
            var mself = this;
            var self = parentSelf;
            if (!callData.EntityStateID && !callData.WorkflowSchemeID && callData.EntityStateName) {
            ...
            //
            if (callData.HTMLDescription)
                mself.Description = ko.observable(callData.HTMLDescription);
            else mself.Description = ko.observable('');
            //
            if (callData.HTMLSolution)
                mself.Solution = ko.observable(callData.HTMLSolution);
            ...
            mself.UtcDateRegisteredJS = callData.UtcDateRegistered;
            //
            ...
            mself.UtcDateOpenedJS = callData.UtcDateOpened;
            //
            ...
            mself.UtcDatePromisedJS = callData.UtcDatePromised;
            //
            if (callData.UtcDatePromised)
            ...
            mself.UtcDateAccomplishedJS = callData.UtcDateAccomplished;
            //
            ...
            mself.UtcDateClosedJS = callData.UtcDateClosed;
            //
            ...
            //
            if (callData.ManhoursString)
                mself.ManHours = ko.observable(callData.ManhoursString)
            else mself.ManHours = ko.observable('');

            if (callData.ManhoursInMinutes)
                mself.ManhoursInMinutes = ko.observable(callData.ManhoursInMinutes)
            else mself.ManhoursInMinutes = ko.observable(0);

            if (callData.ManhoursNormInMinutes)
                mself.ManhoursNorm = ko.observable(callData.ManhoursNormInMinutes)
            else mself.ManhoursNorm = ko.observable(0);

            if (callData.ManhoursNormString)
                mself.ManhoursNormString = ko.observable(callData.ManhoursNormString)
            ...
            mself.UserFieldNamesDictionary = callData.UserFieldNamesDictionary;
            ...
            const uri = `api/calls/${mself.ID()}`;
            mself.getProblems = function ($ctrl) {
                const retD = $.Deferred();

                require(['models/SDForms/Shared/callReferencesFetcher'], function (callRefs) {
                    $.when(callRefs.GetCallProblems(uri, $ctrl)).done(function (result) {
                        if (result.success) {
                            mself.ProblemsCount(result.ids.length);
                            let response = result.ids.map((el) => {
                                return {
                                    CallID: mself.ID(),
                                    ObjectID: el,
                                };
            ...
            mself.addProblem = function (problemID) {
                return mself.addProblems([problemID]);
            }
            mself.addProblems = function (problemIDs, onAdd, $ctrl) {
                const retD = $.Deferred();
                let processed = 0;

                require(['models/SDForms/Shared/callReferencesFetcher'], function (callRefs) {
                    callRefs.ReferenceMultipleProblemsWithCallAsync(
                        uri,
                        problemIDs,
                        $ctrl,
                        function (problemID) {
                        ...
                        function () {
                            mself.ProblemsCount(mself.ProblemsCount() + processed);
                            retD.resolve(processed === problemIDs.length);
                        }
                    );
                });
            ...
            mself.deleteProblems = function (problemIDs, $ctrl, onDelete) {
                const retD = $.Deferred();
                let processed = 0;
                let ids = problemIDs;

                require(['models/SDForms/Shared/callReferencesFetcher'], function (callRefs) {
                    callRefs.RemoveMultipleProblemReferencesWithCallAsync(
                        uri,
                        ids,
                        $ctrl,
                        function (problemID) {
