    ...
    var module = {
        ViewModel: function (negID, objID, objClassID, canEdit_object, regionID, comment) {
            ...
            self.NotStarted = ko.computed(function () {
                return self.Status() === null || self.Status().ID === 0;
            });
            ...
            self.CanEditDD = ko.computed(function () {
                return self.NotStarted() && self.CanEdit() && !self.IsFinanceMode();
            });
            //
            self.CanVote = ko.observable(false);
            self.NotSaved = ko.observable(!negID);
            ...
            self.ajaxControl_loadModes = new ajaxLib.control();
            self.ajaxControl_loadStatuses = new ajaxLib.control();
            ...
            function getModes() {
                ...
                $.when(userD).done(function (user) {
                    self.ajaxControl_loadModes.Ajax(self.$region,
                        {
                            dataType: "json",
                            method: 'GET',
                            url: '/api/negotiationModes'
                        },
                        function (modes) {
                            self.ModeList = modes;
            ...
            function getStatuses() {
                ...
                $.when(userD).done(function (user) {
                    self.ajaxControl_loadStatuses.Ajax(self.$region,
                        {
                            dataType: "json",
                            method: 'GET',
                            url: '/api/negotiationStatuses'
                        },
                        function (modes) {
                            self.StatusList = modes;
            ...
            self.Save = function () {
                //
                var method = self.negotiationID ? "PATCH" : "POST";
                var url = self.negotiationID ? '/api/negotiations/' + self.negotiationID : '/api/' + self.objClassID + '/' + self.objID + '/negotiations';
                var data = {
                    'Name': self.Theme(),
                    'Mode': self.Mode().ID,
                    'UserIDs': self.UsersIDList()
                };
                var PostD = $.Deferred();
            ...
            self.StartNegotiation = function (negArray) {
                ...
                $.when(swalD).done(function () {
                    var promise = negLib.StartNegotiation(
                        self.negotiationID,
                        function () {
                    ...
                    $.when(promise).done(function () { retD.resolve(); });
                });
