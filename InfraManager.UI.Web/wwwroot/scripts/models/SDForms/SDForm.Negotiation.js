define(['knockout', 'jquery', 'ttControl', 'models/SDForms/SDForm.NegotiationVote', 'ajax'], function (ko, $, tclib, voteLib, ajaxLib, imLib) {
    var module = {
        Negotiation: function (imList, obj, isFinanceModeObservable, financeModeTypeObservable, canEdit_object, readOnly_object) {
            ...
            //
            self.ModeString = ko.observable(obj.ModeName);
            self.StatusString = ko.observable(obj.StatusName);
            ...
            self.CompareUser = function (user) {
                ...
                self.UserList().forEach(function (el) {
                    ...
                    if (user.UserID == el.UserID) {
                        ...
                        $.when(el.DeputyUsers.promise).done(
                            function () {
                                el.DeputyUsers.items.forEach(function (elUser) {
                                    if (user.UserID == elUser.ID) {
                                        obj.push(el);
                                    }
                                });
                                userD.resolve();
                            });
                        userPromises.push(userD.promise());
                    }
                })
                $.when.apply($, userPromises).done(function () {
                    retD.resolve(obj);
                });
                return retD.promise();
            };
            //
            if (obj.NegotiationUsers != null) {
                ko.utils.arrayForEach(obj.NegotiationUsers, function (el) {
                    self.UserList().push(new module.NegotiationUser(el, self));
                });
                //
                self.UserList.valueHasMutated();
                $.when(userD).done(function (user) {
                    $.when(self.CompareUser(user)).done(function (obj) {
                        ko.utils.arrayForEach(obj, function (u) {
                            if (u) {
                                u.CanMessage(true);
                        ...
                        swal({
                            ...
                            function (value) {
                                ...
                                if (value == true) {
                                    ...
                                    //
                                    var promise = module
                                        .StartNegotiation(self.ID, function (negotiation) {
                                            $(document).trigger('local_objectUpdated', [160, self.ID, self.ObjectID]);//OBJ_NEGOTIATION для обновления в списке
                                        });
                        ...
                        swal({
                            ...
                            function (value) {
                                ...
                                if (value == true) {
                                    ...
                                    //
                                    var promise = module
                                        .DeleteNegotiation(self.ID, function (negotiation) {
                                            $(document).trigger('local_objectDeleted', [160, self.ID, self.ObjectID]);//OBJ_NEGOTIATION для обновления в списке
                                        });
                                    $.when(promise).done(function () {
                                        hideSpinner();
                                        PostD.resolve();
            ...
            self.Merge = function (obj) {
                ...
                self.CanVote = ko.observable(false);
                //
                self.ModeString(obj.ModeString || obj.ModeName);
                self.StatusString(obj.StatusString || obj.StatusName);
                self.Mode(obj.Mode);
                ...
                if (obj.NegotiationUsers != null) {
                    ...
                    self.UserList.valueHasMutated();
                    $.when(userD).done(function (user) {
                        $.when(self.CompareUser(user)).done(function (el) {
        ...
        NegotiationUser: function (obj, parent) {
            ...
            uself.DeputyUserList = [];
            uself.DeputyUsers = module.DeputyUsers(obj.User);
            $.when(uself.DeputyUsers.promise).done(function () {
                uself.DeputyUserList = uself.DeputyUsers.items;
            });
            if (!obj.Message)
                obj.Message = '';
...
;
                ...
                require(['usualForms', 'models/SDForms/SDForm.User'], function (module, userLib) {
                    ...
                    var options = {
                        ...
                        save: function (data) {
                            var retD = $.Deferred();
                            var patchData = {
                                UserID: data.NewValue ? JSON.parse(data.NewValue).id : null
                            };
                            ...
                            new ajaxLib.control().Ajax(null, {
                                ...
                                url: '/api/negotiations/' + uself.NegotiationID + '/users/' + uself.UserID
                            },
                                function (negotiationUser) {                                    
                                    retD.resolve(true);
        ...
        StartNegotiation: function (id, callback) {
            var retD = $.Deferred();
            ...
            function (negotiation) {
                ...
                retD.resolve({ id: id, success: true });
            },
            function () {
                retD.resolve({ id: id, success: false });
            })

            return retD.promise();
        },
        DeleteNegotiation: function (id) {
            var retD = $.Deferred();
        ...
        DeputyUsers: function (user) {
            var retD = $.Deferred();
            var list = {
                promise: retD.promise(),
                items: []
            };

            if (user) { // TODO: Это все надо отрефакторить, не должно быть таких веток
                new ajaxLib.control().Ajax(null, {
                    dataType: 'json',
                    method: 'GET',
                    url: user
                }, function (data) {
                    list.items = data.UserDeputyList;
                    retD.resolve();
                }, function () {
                    retD.resolve();
                });
