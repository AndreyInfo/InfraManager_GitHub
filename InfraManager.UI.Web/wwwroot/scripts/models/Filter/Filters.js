define(['knockout', 'jquery', 'ttControl', 'models/Filter/FilterElement', 'ajax', 'selectControl', 'dateTimeControl'], function (ko, $, tclib, feLib, ajaxLib, scLib, dtLib) {
    var module = {
        ...
        MigratedViews: [ 'Hardware', 'SoftwareInstallation','Inventories', 'AssetSearch' ],
        ViewModel: function () {
            var self = this;
            ...
            self.SetCurrentFilter = function (newFilter, isTemp) {
                ...
                $.when($isLoaded).done(function () {
                    ...
                    $.when(self.currentTableView()).done(function (curViewInfo) {
                        ...
                        var param = {
                            FilterId: newFilter ? newFilter.ID : (self.currentFilter() ? self.currentFilter().ID : ''),
                            IsTemp: isTemp,
                        ...
                        self.ajaxControl_setCurrentFilter.Ajax(null,
                            {
                                url: '/api/currentfilters/' + curViewInfo.view,
                                method: 'PUT',
                                dataType: 'text',
            ...
            self.Load = function () {
                ...
                $.when(self.currentTableView(), $isLoaded).done(function (curViewInfo) {
                    ...
                    if (module.MigratedViews.includes(curViewInfo.view)) {
                        param = {
                            view: curViewInfo.view,
                        };
                        ajaxSettings.url = '/api/filters?' + $.param(param)
                    }
                    //
                    self.ShowSpinnerFilters();
                    self.ajaxControl_load.Ajax(
                        null,
                        ajaxSettings,
                        function (newData) {
                            if (newData != null) {
            ...
            self.SaveFilter = function (filterID, filterName, isTemp, filterElementsCollection, onNewFilterAction, generalFilter) {
                ...
                if (filterElementsCollection.length <= 0) {
                    ...
                    $.when(self.currentTableView()).done(function (curViewInfo) {
                        self.ShowSpinnerFilters();
                        //
                        var method = filterID ? 'PUT' : 'POST';
                        var url = '/api/filters/' + (filterID ? filterID : curViewInfo.view);
                        var data = {
                            Name: filterName,
                        ...
                        self.ajaxControl_saveFilter.Ajax($('#regionFilters'),
                            ...
                            function (newFilter) {
                                if (newFilter) {
                                    ...
                                    var newFilter = new module.Filter(newFilter, self);
                                    //
