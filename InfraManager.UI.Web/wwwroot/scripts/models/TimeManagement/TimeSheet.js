    ...
    var module = {
        ViewModel: function (timeManagementModule, checkDataInSelectedView) {//общая модель представления табель
            ...
            self.IsTimeSheetItemEval = function (timeSheetItem, searchText, searchMode) {
                ...
                if (classID == 701 && timeSheetItem.LinkObjectOwnerID != self.CurrentUserID && self.Filter_ShowAllMy() == true && self.Filter_ShowProblems() == true)
                    return false;//Заявки, где текущий пользователь - Владелец
                if (classID == 701 && timeSheetItem.LinkObjectExecutorID != self.CurrentUserID && self.Filter_ShowAllMy() == true && self.Filter_ShowCalls() == true)
                    return false;//Заявки, где текущий пользователь - Владелец
                if (classID == 18 && timeSheetItem.LinkObjectClassID == 702 && timeSheetItem.LinkObjectOwnerID != self.CurrentUserID  && self.Filter_ShowAllMy() == true && self.Filter_ShowWorksWithManhours() == true)
                    return false;//Работы, связанные с проблемами
                if (classID == 18 && timeSheetItem.LinkObjectClassID == 701 && (timeSheetItem.LinkObjectExecutorID != self.CurrentUserID || timeSheetItem.LinkObjectOwnerID != self.CurrentUserID ) && self.Filter_ShowAllMy() == true && self.Filter_ShowWorksWithManhours() == true)
                    return false;//Работы, связанные с заявками
                if (classID == 18 && timeSheetItem.LinkObjectClassID == 119 && timeSheetItem.LinkObjectExecutorID != self.CurrentUserID && self.Filter_ShowAllMy() == true && self.Filter_ShowWorksWithManhours() == true)
