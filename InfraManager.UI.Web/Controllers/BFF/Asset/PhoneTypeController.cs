...
namespace InfraManager.UI.Web.Controllers.BFF.Asset
{
    [Authorize]
    [ApiController]
    [Route("api/phone/type")]
    public class PhoneTypeController : ControllerBase
    {
        private readonly IBasicCatalogBLL<PhoneType, PhoneTypeDTO, Guid, PhoneTypeForTable> _basicCatalogBLL;

        public PhoneTypeController(IBasicCatalogBLL<PhoneType, PhoneTypeDTO, Guid, PhoneTypeForTable> basicCatalogBLL)
        {
            _basicCatalogBLL = basicCatalogBLL;
        }

        [HttpGet("item")]
        public async Task<PhoneTypeDTO> GetByIDAsync(Guid id)
        {
            return await _basicCatalogBLL.GetByIDAsync(id, HttpContext.RequestAborted);
        }

        [HttpPost("list")]
        public async Task<PhoneTypeDTO[]> Get([FromBody] BaseFilter filter)
        {
        ...
        [HttpPost("save")]
        public async Task<Guid> GetByIdAsync([FromBody] PhoneTypeDTO model)
        {
