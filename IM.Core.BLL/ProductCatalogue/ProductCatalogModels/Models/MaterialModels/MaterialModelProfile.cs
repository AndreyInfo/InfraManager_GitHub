...
internal sealed class MaterialModelProfile : Profile
{
    public MaterialModelProfile()
    {
        CreateMap<ProductCatalogModelData, MaterialModel>()
            .ForMember(dst => dst.ManufacturerID, mapper => mapper.MapFrom(src => src.VendorID));

        CreateMap<MaterialModel, ProductModelOutputDetails>()
            ...
            .ForMember(dst => dst.Properties, mapper => mapper.MapFrom(src => new MaterialModelAdditionalFields(src)));
    }
}
