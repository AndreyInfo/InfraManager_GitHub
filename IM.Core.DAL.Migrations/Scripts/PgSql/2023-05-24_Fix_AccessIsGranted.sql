create or replace function func_access_is_granted(_classid integer, _id uuid, _ownerid uuid, _ownerclassid integer, _type integer, _propagate boolean) returns boolean
    language plpgsql
as
$$
...
elseif _classid = 101 then --organization
		...
		--owner
return func_access_is_granted(29, '00000000-0000-0000-0000-000000000001', _ownerid, _ownerclassid, _type, true);
...
elseif _classid = 9 then --user
		if _id = '00000000-0000-0000-0000-000000000001' then
			return TRUE;
end if;
		--
...
if not func_access_is_granted(378, _typeID, _ownerid, _ownerclassid, 0, true) then 
	...
	elseif _classid = 33 then --адаптер = ттз;
		if exists(select *
				from adapter d
				inner join asset a on a.device_id = d.int_id
				left join users u on u.identificator = a.user_id
...
select into _tmpID, tmpint, _typeID
    (case when a.room_id<>0 then r.im_obj_id
								when a.network_device_id<>0 then rnd.im_obj_id
								when a.terminal_device_id<>0 then rtd.im_obj_id
								else CAST(null AS UUID) end),
				(case when a.room_id<>0 then 3
								when a.network_device_id<>0 then 3
								when a.terminal_device_id<>0 then 3
								else 0 end),				
...
return func_access_is_granted(tmpint, _tmpID, _ownerid, _ownerclassid, _type, true);
elseif _classid = 34 then --пу = ттз; для логических - ресурсы только из ттз
		if exists(select *
				from peripheral d
				inner join asset a on a.device_id = d.int_id
				left join users u on u.identificator = a.user_id
		...
		--
select into _tmpID, tmpint, _typeID
    (case when p.room_id<>0 then r.im_obj_id
								when p.network_device_id<>0 then rnd.im_obj_id
								when p.terminal_device_id<>0 then rtd.im_obj_id
								else CAST(null AS UUID) end),
				(case when p.room_id<>0 then 3
								when p.network_device_id<>0 then 3
								when p.terminal_device_id<>0 then 3
								else 0 end),
				m.product_catalog_type_id
from peripheral p
