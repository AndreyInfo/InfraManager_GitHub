if object_id('[dbo].[func_AccessIsGranted]') is null
	exec('create function [dbo].[func_AccessIsGranted] (@classID int, @id uniqueidentifier, @ownerID uniqueidentifier, @ownerClassID int, @type tinyint, @propagate bit)
	returns bit
	as 
	begin
		return 0;
...
begin
	...
	begin
		if exists(select * from dbo.ObjectAccess with(nolock) where OwnerID = @ownerID and [Type] = @type and (@propagate = 0 or Propagate = 1) and ClassID = @classID and ObjectID = @id) --organization
			return 1
		--owner
		return dbo.func_AccessIsGranted(29, '00000000-0000-0000-0000-000000000001', @ownerID, @ownerClassID, @type, 1)
	end
