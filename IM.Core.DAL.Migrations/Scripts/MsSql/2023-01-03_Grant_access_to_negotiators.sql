if exists (select *
           from   sys.objects
           where  object_id = OBJECT_ID(N'[dbo].[func_userInNegotiation]')
                  AND type IN ( N'FN', N'IF', N'TF', N'FS', N'FT' ))
  drop function [dbo].[func_userInNegotiation]
go

create function [dbo].[func_userInNegotiation](
...
as
begin
	if 303 not in (select ro.[OperationID] from [dbo].[UserRole] ur join [dbo].[RoleOperation] ro on ur.RoleID = ro.RoleID where ur.UserID = @userID)
	begin
		return 0;
	end
	...
	if exists(
		select 1
		from [dbo].[NegotiationUser] nu
		join [dbo].[Negotiation] n on n.ID = nu.NegotiationID		
		where n.ObjectID = @objectID
			and n.ObjectClassID = @objectClassID
			and (
				nu.UserID = @userID
					or @userID in (select du.ChildUserID from [dbo].[DeputyUser] du where du.ParentUserID = nu.UserID 
						and du.UtcDataDeputyWith <= GETUTCDATE()
						and du.UtcDataDeputyBy >= GETUTCDATE())))
	begin
		return 1;
	end
...
if exists (select *
           from   sys.objects
           where  object_id = OBJECT_ID(N'[dbo].[func_AccessIsGranted]')
                  AND type IN ( N'FN', N'IF', N'TF', N'FS', N'FT' ))
  drop function [dbo].[func_AccessIsGranted]
go

create function [dbo].[func_AccessIsGranted] (@classID int, @id uniqueidentifier, @ownerID uniqueidentifier, @ownerClassID int, @type tinyint, @propagate bit)
	returns bit
as
begin
	...
	select TOP 1 @WorkOrderInitiatorID = InitiatorID, @WorkOrderExecutorID = ExecutorID, @WorkOrderAssignorID = AssignorID, @WorkOrderQueueID = QueueID from dbo.WorkOrder with(nolock) where ID = @id
		...
		begin
			...
			if (@ownerClassID = 9 and 
				...
				if [dbo].[func_userInNegotiation](@id, @classID, @ownerID) = 1
				begin
					return 1
				end
		end
	...
	begin
		...
		if @ownerClassID = 9 and [dbo].[func_userInNegotiation](@id, @classID, @ownerID) = 1
		begin
			return 1
		end
...
--				return 0   --move up
	...
	begin
		if exists(select * from dbo.ObjectAccess with(nolock) where OwnerID = @ownerID and [Type] = @type and (@propagate = 0 or Propagate = 1) and ClassID = @classID and ObjectID = @id) --serviceCategory
			return 1
		--serviceCatalogue
		return dbo.func_AccessIsGranted(127, '00000000-0000-0000-0000-000000000000', @ownerID, @ownerClassID, @type, 1)
	end
	...
	if exists(select * from dbo.ObjectAccess with(nolock) where OwnerID = @ownerID and [Type] = @type and (@propagate = 0 or Propagate = 1) and ClassID = @classID and ObjectID = @id) --current category
		...
		--
		--deviceCatalogue
		return dbo.func_AccessIsGranted(30, '00000000-0000-0000-0000-000000000000', @ownerID, @ownerClassID, @type, 1)
	end
