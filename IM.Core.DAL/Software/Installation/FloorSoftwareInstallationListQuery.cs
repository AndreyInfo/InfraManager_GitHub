...
namespace InfraManager.DAL.Software.Installation
{
    ...
    internal class FloorSoftwareInstallationListQuery : ISoftwareInstalationListQuery
    {
        private readonly CrossPlatformDbContext _db;

        public FloorSoftwareInstallationListQuery(CrossPlatformDbContext db)
        {
            _db = db;
        }
        ...
        public async Task<System.Linq.IQueryable<ViewSoftwareInstallation>> QueryAsync(SoftwareInstallationListFilter filter, CancellationToken token = default)
        {
            ...
            var queryList =
                      from floor in _db.Set<Floor>()
                      join room in _db.Set<Room>() on floor.ID equals room.Floor.ID
                        into roomLeftGroup
                      ...
                      where floor.IMObjID == filter.TreeSettings.FiltrationObjectID
