...
{
    internal class WorkOrderBudgetLookupQuery : ILookupQuery
    {
        ...
        public WorkOrderBudgetLookupQuery(
        ...
        public async Task<ValueData[]> ExecuteAsync(CancellationToken cancellationToken = default)
        {
            var query = from budgetUsage in _budgetUsages
                        ...
                        where budgetUsage.ObjectClass == ObjectClass.WorkOrder
                            && _workOrders.Any(p => p.IMObjID == budgetUsage.ObjectId)
                        select new
                        {
