...
internal class AdapterQuery : IAdapterQuery, ISelfRegisteredService<IAdapterQuery>
{
    ...
    public async Task<AdapterItem[]> ExecuteAsync(PaggingFilter filter, IOrderedQueryable<Adapter> orderedQuery, CancellationToken cancellationToken)
    {
        var query = orderedQuery.Select(x => new AdapterItem
        {
            ID = x.ID,
